...
export default function ProjectsGrid() {
    ...
    return (
        <section className="relative z-10 py-24 border-t border-white/5 bg-[#0c1118]">
            <div className="max-w-6xl mx-auto px-6 md:px-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
                    {projectsList.map((project) => {
                        ...
                        const githubLink = projectLinks[project.linkKey] || "#";
                        const liveLink = project.liveLinkKey ? projectLinks[project.liveLinkKey] : null;

                        return (
                            <motion.div
                                key={project.id}
                                whileHover={{ y: -5 }}
                                className="group relative p-8 rounded-2xl bg-white/2 border border-white/5 hover:border-blue-500/30 transition-all overflow-hidden flex flex-col"
                            >
                                <div className="flex-1">
                                    {/* Выводим статус (Active, Deployable и т.д.) */}
                                ...
                                <div className="flex items-center gap-4 mt-auto">
                                    <a href={githubLink} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-gray-500 hover:text-white transition-colors" title="GitHub">
                                    ...
                                    {liveLink && (
                                        <a href={liveLink} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-blue-400 hover:text-blue-300 transition-colors" title="Live Demo">
            ...
            {/* Плавный градиент в черный внизу секции (на всю ширину экрана) */}
